...
const getMermerVersion = () => {
  let version = null;
  const exePath = getExecutablePath('mermer');
  // first see if we can get a mermer exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or mermer is on the path
    try {
      version = child_process

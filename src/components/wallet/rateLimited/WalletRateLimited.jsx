...
import { AlertDialog, Card, Flex } from '@mermer/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const mermerper_input = ip_parsed.limit;
    ...
    const mermerper_value = Number.parseInt(Number(mermerper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (mermer per interval):{' '}
                  {mojo_to_mermer_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = mermer_to_mojo(fee_input.value);

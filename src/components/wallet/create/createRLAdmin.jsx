...
import { AlertDialog } from '@mermer/core';
import {
  Typography,
  Button,
  Box,
...
import { mermer_to_mojo } from '../../../util/mermer';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      mermerper_input.value === '' ||
      Number(mermerper_input.value) === 0 ||
      !Number(mermerper_input.value) ||
      isNaN(Number(mermerper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const mermerper = mermer_to_mojo(mermerper_input.value);
    const mermerper_value = Number.parseInt(Number(mermerper));
    const userpubkey = userpubkey_input.value;
    const amount = mermer_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = mermer_to_mojo(fee_input.value);

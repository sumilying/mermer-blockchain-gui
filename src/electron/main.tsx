...
import mermerEnvironment from '../util/mermerEnvironment';
import mermerConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!mermerEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    mermerConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!mermerConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !mermerConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (mermerConfig.manageDaemonLifetime()) {
        mermerEnvironment.startMermerDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Mermer Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Mermer-Network/mermer-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Mermer-Network/mermer-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Mermer-Network/mermer-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Mermer-Network/mermer-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Mermer-Network/mermer-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/mermer_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/mermer_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Mermer Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Mermer' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Mermer Blockchain' }),
            click: () => {
              openAbout();
            },
          },

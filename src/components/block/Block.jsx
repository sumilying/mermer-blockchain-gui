...
export default function Block() {
  ...
  const poolReward = mojo_to_mermer(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_mermer(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const mermerFees = blockRecord.fees
    ? mojo_to_mermer(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.mermerexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.mermerexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: mermerFees ? `${mermerFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Mermer blockchain
            </Trans>
          </Back>
        }
        action={

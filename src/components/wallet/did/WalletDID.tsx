...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@mermer/core';
import {
...
import { mojo_to_mermer_string } from '../../../util/mermer';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_mermer_string(props.balance)} TMMH
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'mermer_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'mermer_addr://') {
